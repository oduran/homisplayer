(function($) {
	$.fn.bilimtekWeather = function( options ){
		// Initializing
		if( (typeof(options) ).match('object|undefined') ){
			return this.each(function(i){
				new $.bilimtekWeather(this, options);
			});
		}else{
			return this.each(function(i){
				// Change function
				var wsData = $(this).data('bilimtekWeather');
	...
	$.bilimtekWeather = function(el, options) {
		var ws = this;
		ws.$el = $(el).addClass('ws-container');
		ws.$el.data('bilimtekWeather', ws);

		ws.init = function(){

			// Setting options (user settings) and global (not modificable) parameters

			ws.o = $.extend( {}, $.bilimtekWeather.options, options );
			...
			if( ws.o.language ){
				...
				ws.l = $.extend( {}, $.bilimtekWeather.language );
			}
		...
		ws.reInit = function(){
			...
			$('<div><span class="ws-getting">'+ws.l.get+':</span><br><span class="ws-curlocation"></span></div>').appendTo($(el)).addClass('ws-loading');
			...
			if( ws.o.WWOAPIKey == 'your_api_key' && !ws.o.ajaxURL ){
				alert('bilimtekWeather:\r\n\r\nYou must sign up to get your own WorlWeatherOnline API key! Of course the registraion is free:\r\n\r\nhttp://developer.worldweatheronline.com/member/register');
			}else{
				ws.change(ws.g.curID);
			}
		};

		// CHANGED GEOLOCATION feature v2.4.5 (we must use HTML5 / Google Maps API, because MaxMind geolocation service is no more free)

		ws.geoLocation = function(id,search){
			...
			if( !ws.g.curLocation ){
				...
				if( typeof google != 'undefined' && typeof google.maps != 'undefined' ){
					...
					// If HTML5 Geolocation is not available, trying to locate with Google JSAPI...

					var geoFallback = function(){
						...
						$.getScript('https://www.google.com/jsapi', function(){
							...
							ws.animateInfobox(id,search);
						});
					};

					// Trying to locate with HTML5 geolocation...

					if( navigator.geolocation ){
						navigator.geolocation.getCurrentPosition(geoSuccess, geoFallback);
					}
		...
		ws.change = function(id,search){
			...
			if( ws.g.locations[id].indexOf('GEOLOCATION') != -1 ){
				ws.geoLocation(id,search);
			}else{
				ws.animateInfobox(id,search);
			}
		};
		...
		ws.animateInfobox = function(id,search){
		...
		ws.getWeather = function(id,search){
			...
			// IMPROVEMENT v2.6.0 Added premium WWO API Key support
			// IMPROVEMENT v2.6.3 Added WWO API Version support

			ws.g.type = 'free';

			if( ws.o.premiumAPIKey === true ){
				ws.g.type = 'premium';
			}

			ws.g.version = 2;

			if( ws.o.premiumAPIKey === true || ws.o.WWOAPIVersion === 1 ){
				ws.g.version = 1;
			}

			if( typeof ws.o.WWOAPIVersion === 'number' ){
				ws.g.version = ws.o.WWOAPIVersion;
			}
			...
			$(el).find('.ws-getting').css({
				display: 'inline'
			});
			$(el).find('.ws-curlocation').text('');
			$(el).find('.ws-loading .ws-curlocation').html(lname);
			$(el).find('.ws-loading').fadeIn(500);
			ws.g.urlModifier = (ws.o.forecastHour===0?'weather':'forecast');
			if( ws.o.ajaxURL ){
				var url = ws.o.ajaxURL;
				var obj = { action : 'bilimtekWeather_getdata', location : l, service : 'weather' };
			}else{
			 	url  = ws.g.SSL + '://api.openweathermap.org/data/2.5/'+ws.g.urlModifier;
				url += '?APPID='+ws.o.WWOAPIKey+'';
				url += '&q='+encodeURIComponent(l);
				url += '&lang='+ws.o.lang;
				url += '&units='+ws.o.units;
				url += '&lat=' + 39.913539
				url += '&lon=' + 32.790359
				obj = {};
			}
			...
			var CtoF = function(celcius){
				return  celcius.value * 9 / 5 + 32;
			}

			var degToDir = function(degree){
				var directions = ["N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE",
                       "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW"];
				var i = (degree + 11.25)/22.5;
				return directions[Math.round(i % 16)];
			}
			function weatherSuccess(s)
			{
					/*
					 	@uygar
					 	27.11.2015
					...
					if( typeof data !== 'undefined' && typeof data.error !== 'undefined' && typeof data.error[0] !== 'undefined' && typeof data.error[0].msg !== 'undefined'  ){
						if( typeof console !== 'undefined' ){
							console.log( '\r\n%cbilimtekWeather Error: '+data.error[0].msg+'\r\n', 'color: white; background: red; padding: 10px; font-size: 14px; border-radius: 3px;' );
							ws.g.error = 'bilimtekWeather Error: '+data.error[0].msg;
						}
					}
					if(
						typeof s !== 'undefined' && typeof s.results !== 'undefined' && typeof s.results.error !== 'undefined' && typeof s.results.error.type !== 'undefined' && typeof s.results.error.message !== 'undefined'
					){
						if( typeof console !== 'undefined' ){
							console.log( '\r\n%cOpenWeatherMap '+s.cod+': '+s.message+'\r\n', 'color: white; background: red; padding: 10px; font-size: 14px; border-radius: 3px;' );
							ws.g.error = 'bilimtekWeather '+s.cod+': '+s.message;
						}
					}

					if(ws.o.forecastHour === 0 
					...
					var locationName = "Ankara";
					if(typeof(data["name"]) !== 'undefined' && data["name"] != null)
					{
						locationName = data["name"];
					}
					$(".ws-location").html(locationName);
					...
					if( String(data['cod']) ==='200' ){
						...
						var wData = {
							...
							current_condition : {
								weather : {
									condition : ws.l[ws.conversion[cur['weather'][0]['id']]],
									icon : cur['weather'][0]['icon'],
									code : ws.conversion[cur['weather'][0]['id']]
								},
								temperature : 
								{
									metric : Math.round(cur['main']['temp']),
									imperial : Math.round(CtoF(cur['main']['temp']))  // TODO: imperial'da hata var
								},
								...
								wind : {
									speed : {
										metric : cur['wind']['speed'],
										imperial : KMtoMI(cur['wind']['speed'])
									},
									direction : degToDir(cur['wind']['deg'])
								},
								pressure : cur['main']['pressure'],
								visibility : {
									metric : '', // TODO: visibility forecast'te yok, current weather icin farkli url yazalim
						...
						wData['forecast'] = [];
						// 3-6-9-12 hour forecast
						if (ws.o.forecastHour!==0){
							for(var f=0;f<4;f++) {
								fore = data['list'][f + 1]; //0: current 1: 3 hour later
								// IMPROVEMENT v2.6.0 Added premium WWO API Key support
								wData.forecast[f] = {
									temperature: {
										high: {
											metric: fore['main'].temp_max,
											imperial: CtoF(fore['main'].temp_max)
										},
										low: {
											metric: fore['main'].temp_min,
											imperial: CtoF(fore['main'].temp_min)
										}
									},
									weather: {
										code: ws.conversion[fore['weather'].id],
										condition: fore['weather'].description // WS icinden almak istersek: ws.l[ws.conversion[fore['weather'].id]]
									}
								}
							}
						}else{
							weatherWidgetObj.sunrise = cur['sys'].sunrise;
							weatherWidgetObj.sunset = cur['sys'].sunset;
						}
						var curDay = new Date();
						if(ws.o.forecastHour===0){
							wData['day'] = curDay.getDay();
						}else {
							curDay.setHours(curDay.getHours()+ws.o.forecastHour);
							curDay.setMinutes(0); //saat basini al, widget oyle yapiyor
							wData['day'] = curDay.getDay();
						}
						wData['time'] = curDay.getHours+':'+curDay.getMinutes();

						if(curDay.getTime()<(weatherWidgetObj.sunset*1000) && curDay.getTime()>=(weatherWidgetObj.sunrise*1000)){
							wData['current_condition']['daytime'] = 'day';
						}else{
							wData['current_condition']['daytime'] = 'night';
						}

						var wCode = wData['current_condition']['weather']['code'];
						if( wCode == '113'){
					...
					else
					{
						...
						if( ws.g.error ){
							...
							$(el).find('.ws-curlocation').text(ws.g.error);
							ws.g.error = false;
						}else{
							$(el).find('.ws-curlocation').html(ws.l.not+':<br>'+lText);
						}
		...
		ws.parseData = function(wData,id,lname){
			...
			if(ws.o.forecastHour===0){
				...
				tempmax = wData.forecast[0].temperature.high[ws.o.measurement];
				tempmin = wData.forecast[0].temperature.low[ws.o.measurement];
			...
			if( ws.o.windDirection == 'right' ){
				...
				{
					console.log("No wind direction error");
				}
			}
		...
		ws.prevNext = function(id){
			...
			if( ws.g.locations.length > 1 ){
				...
				var curLoc = ( ws.g.curLocation && ws.g.curLocation != ws.l.una ) ? ws.g.curLocation.split(',')[0] +' (' + ws.l.cl + ')' : ws.l.cl;

				var ptext = ws.g.locations[prev].split(',')[0].indexOf('GEOLOCATION') != -1 ? curLoc : ws.g.locations[prev];
				var ntext = ws.g.locations[next].split(',')[0].indexOf('GEOLOCATION') != -1 ? curLoc : ws.g.locations[next];
				...
				$(el).find('.ws-prev-arrow').click(function(e){
					e.preventDefault();
					$(this).mouseleave();
					$(el).bilimtekWeather(prev);
				});
				$(el).find('.ws-next-arrow').click(function(e){
					e.preventDefault();
					$(this).mouseleave();
					$(el).bilimtekWeather(next);
				});
	...
	$.bilimtekWeather.language = {
		cl					: 'Şu andaki yeriniz',
		ht					: 'Yüksek',
		lt					: 'Düşük',
		hu					: 'Nem',
		pr					: 'Yağış',
		...
		// Weather condition names
		  ...
		  '122'          : 'Çok bulutlu',
		  '143'          : 'Puslu',
		  '176'          : 'Yakında parçalı yağmur',
		  '179'          : 'Yakında parçalı kar',
		  '182'          : 'Yakında karla karışık yağmurlu',
		  '185'          : 'Yakında donan yağmur',
		  '200'          : 'Yakında gök gürültülü yağış',
		  '227'          : 'Tipi',
		  '230'          : 'Kar fırtınası',
		  '248'          : 'Sisli',
		  '260'          : 'Donan sis',
		  '263'          : 'Düzensiz ince yağan yağmurlar',
		  '266'          : 'İnce yağan yağmurlar',
		  '281'          : 'Donan yağmur',
		  '284'          : 'Şiddetli donan yağmur',
		  '293'          : 'Parçalı hafif yağış',
		  '296'          : 'Hafif yağışlı',
		  '299'          : 'Zaman zaman orta yoğunlukta yağış',
		  '302'          : 'Orta yoğunlukta yağış',
		  '305'          : 'Zaman zaman şiddetli yağmur',
		  '308'          : 'Şiddetli yağmur',
		  '311'          : 'Hafif soğuk yağış',
		  '314'          : 'Soğuk yağış',
		  '317'          : 'Hafif sulusepken',
		  '320'          : 'Sulusepken',
		  '323'          : 'Hafif karlı',
		  '326'          : 'Hafif karlı',
		  '329'          : 'Orta şiddetli kar',
		  '332'          : 'Orta şiddetli kar',
		  '335'          : 'Şiddetli kar',
		  '338'          : 'Şiddetli kar',
		  '350'          : 'Ice pellets',
		  '353'          : 'Hafif yağmur',
	...
	/*
		@uygar
		27.11.2015
		Open Weather Map hava durumu kodlarinin World weather online kodlarina donusumu
	 	$.bilimtekWeather.conversion
	 */
	$.bilimtekWeather.conversion = {
		'200' : '386',
		'201' : '389',
	...
	$.bilimtekWeather.options = {
		...
		// NEW FEATURES v2.6.2
		useHTTPS			: false,
		// Premium WWO API Key support
		premiumAPIKey : false,
		// NEW FEATURE v2.6.3
		WWOAPIVersion : 2,
		// Important! You must sign up to get your own WorldWeatherOnline API key!
		// Please do NOT use our API key, except for testing only!
		// The registration for your API key is free:
		// http://developer.worldweatheronline.com/member/register
