...
(function($, window, document, undefined) {
    ...
    $.fn.bilimteksocialmediapictureslider = function(_options) {
        moment.locale('tr');
        var defaults = {
            plugin_folder: '', 
        ...
        var options = $.extend(defaults, _options),
            container = $(this),
            template,
            social_networks = ['facebook', 'instagram', 'vk', 'google', 'blogspot', 'twitter','foursquare'],
        ...
        var Feed = {
            ...
            foursquare: {
                ...
                getData: function(account) {
                    var url;
                    url = Feed.foursquare.api + options.foursquare.accounts+ '/tips?sort=recent'+ '&oauth_token='+options.foursquare.access_token + '&v=20150901&';
                    //var username = account.substr(1);
                    Utility.get_request(url, Feed.foursquare.utility.getPosts);
                },
                utility: {
                    getPosts: function(json) {
                        var venuex = json.response.tips; 
                        $.each(json.response.tips.items, function() {
                            var allTips = this;
                            var post = new SocialFeedPost('foursquare', Feed.foursquare.utility.unifyPostData(venuex, allTips));

                            post.render();

                        });
					},
                    unifyPostData: function(venuex, element) {
                        var post = {};
                        post.id = element.id;
                        post.dt_create = moment(parseInt(element.createdAt)*1000);
                        post.author_link = venuex.canonicalUrl;
                        var xx = element.user.photo;   
                        post.author_picture = xx.prefix + '36x36'+(xx.suffix);  
                        post.author_name = element.user.firstName + " " + (typeof element.user.lastName === 'undefined'?'':element.user.lastName);   //venuex.name;
                        post.name ="asd";
                        post.message = element.text; 
                        post.description =  '';
                        post.likes = element.likes.count + " beğenme";
                        post.link = venuex.canonicalUrl; 
                        return post;
                    }
                }
            }
